...
//unpack
func Unpack(buffer []byte, readerChannel chan []byte, endian string) []byte {
	length := len(buffer)
	//var i int
	...
	if head == ConstHeader {
		...
		var numRecord int 
		if endian =="big" {
			numRecord = BytesToInt16_Big(buffer[ConstHeaderLength+ConstMacLength : ConstHeaderLength+ConstMacLength+ConstRecordLength])
		...
		if totLength != length {
			...
			DeviceId:=PqStorage.GetDeviceId(Macaddr)
			var rssData []*Rssiample
			for i := ConstHeaderLength + ConstMacLength + ConstRecordLength; i < totLength-1;  {
				data := buffer[i : i+12]
				i = i + 12
				//passbyte:= make([]byte,18)
				//copy(passbyte[0:],data)
				//copy(passbyte[12:],buffer[ConstHeaderLength : ConstHeaderLength+ConstMacLength])
				readerChannel <- data
				//readerChannel <- passbyte
				
				if  endian == "big"{
					rssData = append(rssData,&Rssiample{
						//Id:0,
						Ts:BytesToInt_Big(data[0:4]),
						Imac:int64(DeviceId),
						Dmac:int64(BytesToMac(data[4:10])),
						Rss:int(BytesToInt8_Big(data[10:11]))*(-1),
						Frq:int(BytesToInt8_Big(data[11:12])),
					})
				}else{
					rssData = append(rssData,&Rssiample{
						//Id:0,
                                                Ts:BytesToInt(data[0:4]),
                                                Imac:int64(DeviceId),
                                                Dmac:int64(BytesToMac(data[4:10])),
                                                Rss:int(BytesToInt8(data[10:11]))*(-1),
                                                Frq:int(BytesToInt8(data[11:12])),
                                        })

				}
			}
			readerChannel <-buffer[totLength-1:totLength]
			RedisInst.SaveRssi(rssData)
			return make([]byte, 0)
		}
	}
